...
def get_data(pair):
    ...
    aimd = {
        "r": np.loadtxt(get_txt_file("aimd/partial_master", f"r_random_{pair}.txt")),
        "t": np.loadtxt(get_txt_file("aimd/partial_master", f"t_random_{pair}.txt")),
        "g": np.loadtxt(
            get_txt_file("aimd/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "optB88",
    }

    spce = {
        "r": np.loadtxt(get_txt_file("spce/partial_master", f"r_random_{pair}.txt")),
        "t": np.loadtxt(get_txt_file("spce/partial_master", f"t_random_{pair}.txt")),
        "g": np.loadtxt(
            get_txt_file("spce/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "SPC/E",
    }
    ...
    tip3p_ew = {
        "r": np.loadtxt(
            get_txt_file("tip3p_ew/partial_master", f"r_random_{pair}.txt")
        ),
        "t": np.loadtxt(
            get_txt_file("tip3p_ew/partial_master", f"t_random_{pair}.txt")
        ),
        "g": np.loadtxt(
            get_txt_file("tip3p_ew/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "TIP3P_EW",
    }

    bk3 = {
        "r": np.loadtxt(get_txt_file("bk3/partial_master", f"r_random_{pair}.txt")),
        "t": np.loadtxt(get_txt_file("bk3/partial_master", f"t_random_{pair}.txt")),
        "g": np.loadtxt(get_txt_file("bk3/partial_master", f"vhf_random_{pair}.txt")),
        "name": "BK3",
    }

    reaxff = {
        "r": np.loadtxt(
            get_txt_file("reaxff/partial_master", f"r_random_{pair}.txt")
        ),
        "t": np.loadtxt(
            get_txt_file("reaxff/partial_master", f"t_random_{pair}.txt")
        ),
        "g": np.loadtxt(
            get_txt_file("reaxff/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "CHON-2017_weak",
    }
    ...
    aimd_330 = {
        "r": np.loadtxt(
            get_txt_file("aimd/330k/partial_master", f"r_random_{pair}.txt")
        ),
        "t": np.loadtxt(
            get_txt_file("aimd/330k/partial_master", f"t_random_{pair}.txt")
        ),
        "g": np.loadtxt(
            get_txt_file("aimd/330k/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "optB88 (330K)",
    }
    ...
    dftb = {
        "r": np.loadtxt(get_txt_file("dftb/partial_master", f"r_random_{pair}.txt")),
        "t": np.loadtxt(get_txt_file("dftb/partial_master", f"t_random_{pair}.txt")),
        "g": np.loadtxt(
            get_txt_file("dftb/partial_master", f"vhf_random_{pair}.txt")
        ),
        "name": "3obw",
    }
...
def plot_peak_subplots(datas):
    ...
    plt.tight_layout()
    fig.savefig("master_figures/partial_peak_decay.png", dpi=500, bbox_inches="tight")
    fig.savefig("master_figures/partial_peak_decay.pdf", dpi=500, bbox_inches="tight")
...
if __name__ == "__main__":
    for pair in pairs:
        ...
        if pair == "O_H":
            plot_oh_peak(datas, ylim=ylim, filename=f"master_figures/O_H_hbond_peak.png")
            plot_oh_peak(datas, ylim=ylim, filename=f"master_figures/O_H_hbond_peak.pdf")
            # first_oh_peak(datas,filename=f'master_figures/{pair}_first_peak.png')
            # first_oh_peak(datas,filename=f'master_figures/{pair}_first_peak.pdf')

        plot_vhf_subplots(datas, ylim=ylim, filename=f"master_figures/{pair}_subplot.pdf")
        plot_vhf_subplots(datas, ylim=ylim, filename=f"master_figures/{pair}_subplot.png")

        # if pair == 'H_H':
        #    first_peak_height(datas, peak_guess=peak_guess, ylim=(0.8, 1.8), filename=f'master_figures/{pair}_first_peak.pdf', shift=False)
        #    first_peak_height(datas, peak_guess=peak_guess, ylim=(0.8, 1.8), filename=f'master_figures/{pair}_first_peak.png', shift=False)
        # if pair == 'H_H':
        #    first_second_peak(datas, first_peak_guess=0.25, second_peak_guess=0.4, filename=f'master_figures/{pair}_first_second.pdf')
        #    first_second_peak(datas, first_peak_guess=0.25, second_peak_guess=0.4, filename=f'master_figures/{pair}_first_second.png')
        # if pair == 'O_H':
        #    first_peak_min(datas, peak_guess=0.2, filename=f'master_figures/{pair}_minima.pdf')
        #    first_peak_min(datas, peak_guess=0.2, filename=f'master_figures/{pair}_minima.png')
        # if pair == 'O_O':
        #    first_peak_height(datas, peak_guess=0.3, ylim=(0.01, 2.5), filename=f'master_figures/{pair}_first_peak.pdf')
        #    first_peak_height(datas, peak_guess=0.3, ylim=(0.01, 2.5), filename=f'master_figures/{pair}_first_peak.png')
