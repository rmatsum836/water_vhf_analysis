...
def plot_first_peak_subplot(datas, si=False):
    ...
    for data in datas:    
        ...
        if data['name'] == 'IXS':
            ax.semilogy(data['t'], maxs-1, '.', lw=4, label=data['name'], color=get_color(data['name']))
        else:
            ax.semilogy(data['t'], maxs-1, ls='--', lw=3, label=data['name'], color=get_color(data['name']))
    if si:
    ...
    fig.legend(bbox_to_anchor=(0.45, 1.10), loc='upper center', prop={'size': fontsize}, ncol=4)
...
def plot_fits():
    ...
    for i in range(len(df)):
        ...
        ax.plot(time, data["A_1"]*np.exp(-(time/data["tau_1"])**data["gamma_1"]),
...
def plot_second_subplot(datas):
    ...
    ax = axes[0]
    ax.text(-0.10, 0.95, 'a)', transform=ax.transAxes,
            size=20, weight='bold')
    ax.set_prop_cycle('color', colors)
    for data in datas:
        print(data['name'])
    ...
    ax = axes[1]
    ax.text(-0.10, 0.95, 'b)', transform=ax.transAxes,
            size=20, weight='bold')
    for data in datas:
        print(data['name'])
        # Find nearest value to 0.1
        ...
        if data['name'] == 'IXS':
            ...
            ax.plot(data['t'], min_max, '.', lw=4, label=data['name'], color=get_color(data['name']))
        else:
            #ax.plot(data['t'], (maxs-1)/(maxs[0]-1), ls='--', lw=2, label=data['name'], color=get_color(data['name']))
            ax.plot(data['t'], min_max, ls='--', lw=3, label=data['name'], color=get_color(data['name']))
    ax.set_xlim((0.005, 0.8))
